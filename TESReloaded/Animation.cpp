...
ActorAnimData* Animation::TrackNewActorAnimData() {	
	...
	void* (__cdecl * Oblivion_MemAlloc)(size_t) = (void*(*)(size_t))0x00401F00;
	NiTList<BSAnimGroupSequence>* ORAnims = (NiTList<BSAnimGroupSequence>*)Oblivion_MemAlloc(0x10);
...
void CreateAnimationHook()
{
	...
	*((int *)&NewAnimSequenceMultiple)	= 0x00473D90;
	...
	// Extends the ActorAnimData allocation from size 0xDC to 0xE0 (for each constructor call) to store the NiTList<BSAnimGroupSequence>*
	SafeWrite32(0x004E3814, 0xE0);
	SafeWrite32(0x004E3ADD, 0xE0);
	SafeWrite32(0x00667E23, 0xE0);
	SafeWrite32(0x00669258, 0xE0);

	SafeWrite8(0x00474140, 0x0C);	// Extends the AnimSequenceSingle allocation from size 0x08 to 0x0C to store the BSAnimGroupSequence*
	SafeWrite8(0x004741C0, 0x0C);	// Extends the AnimSequenceMultiple allocation from size 0x08 to 0x0C to store the BSAnimGroupSequence*
